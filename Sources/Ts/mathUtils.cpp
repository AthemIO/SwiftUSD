...
static double _SolveCubic_RegulaFalsi(const TsTime poly[4], TsTime y, const GfInterval &bounds)
{
                               ...
                               const GfInterval &bounds)
{
  ...
  for (int i = 0; i < NUM_ITERS; ++i) {
    double dx = (Ts_EvalCubic(poly, x) - y) / Ts_EvalQuadratic(polyDeriv, x);
    x -= dx;
...
// Solve f(x) = y for x where f is a cubic polynomial.
double Ts_SolveCubic(const TsTime poly[4], TsTime y)
{
