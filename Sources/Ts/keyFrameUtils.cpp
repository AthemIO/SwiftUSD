...
const TsKeyFrame *Ts_GetClosestKeyFrame(const TsKeyFrameMap &keyframes, const TsTime targetTime)
{
  ...
  TsKeyFrameMap::const_iterator it = keyframes.lower_bound(targetTime);
...
std::pair<const TsKeyFrame *, const TsKeyFrame *> Ts_GetClosestKeyFramesSurrounding(
    const TsKeyFrameMap &keyframes, const TsTime targetTime)
{
  ...
  TsKeyFrameMap::const_iterator itAtOrAfter = keyframes.lower_bound(targetTime);
                            ...
                            const VtValue &defaultValue)
{
  ...
  if (keyFrame.GetIsDualValued() && !Ts_IsClose(keyFrame.GetLeftValue(), keyFrame.GetValue())) {
    return false;
  }
  ...
  if (prev && next) {
    if (keyFrame.GetKnotType() == TsKnotHeld && prev->GetKnotType() == TsKnotHeld &&
        prev->GetValue() == keyFrame.GetValue())
    {
      ...
      return Ts_IsSegmentFlat(*prev, keyFrame) && Ts_IsSegmentFlat(keyFrame, *next);
    }
  }
  else if (!prev && next) {
...
bool Ts_IsSegmentFlat(const TsKeyFrame &kf1, const TsKeyFrame &kf2)
{
  ...
  const VtValue &v2 = (kf2.GetIsDualValued()) ? kf2.GetLeftValue() : kf2.GetValue();
  ...
  //
  if (kf1.HasTangents() && !Ts_IsClose(kf1.GetValueDerivative(), kf1.GetZero())) {
    return false;
  }

  if (kf2.HasTangents() && !Ts_IsClose(kf2.GetLeftValueDerivative(), kf2.GetZero())) {
    return false;
  }
