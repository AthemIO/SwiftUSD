...
void _LoopResolver::_ResolveInner()
{
    ...
    const GfInterval masterInterval = lp.GetMasterInterval();
    ...
    TF_DEBUG_MSG(
        ...
        masterInterval.GetMin(),
        masterInterval.GetMax(),
    ...
    // Calculate loop iteration counts from repeat frames for legacy code.
    const TsTime period = lp.GetPeriod();
    const int numPreLoops = (period > 0) ?
        static_cast<int>(lp.GetPreRepeatFrames() / period) : 0;
    const int numPostLoops = (period > 0) ?
        static_cast<int>(lp.GetRepeatFrames() / period) : 0;

    // Handle evaluation in echo regions.
    if (loopedInterval.Contains(_evalTime)
        && !masterInterval.Contains(_evalTime))
    {
        const TsTime protoSpan = masterInterval.GetSize();
        const TsTime protoStart = masterInterval.GetMin();
        const TsTime protoEnd = masterInterval.GetMax();

        // Handle evaluation in pre-echo.
        if (_evalTime < protoStart)
        {
            // Figure out which pre-iteration we're in.
            const TsTime loopOffset = protoStart - _evalTime;
            ...
            {
                _valueOffset -= iterNum * lp.GetValueOffset();
            }
        }
        ...
        else
        {
            ...
            const TsTime loopOffset = _evalTime - protoEnd;
    ...
    else if (_evalTime < _firstTime)
    {
        if (_firstTimeLooped)
        {
            ...
            else
            {
                ...
                _extrapKnot2 = _CopyProtoKnotData(
                    _firstInnerProtoIndex, -numPreLoops + 1);
            }
        }
    ...
    else if (_evalTime > _lastTime)
    {
        if (_lastTimeLooped)
        {
            ...
            _extrapKnot1 = _CopyProtoKnotData(
                _firstInnerProtoIndex, numPostLoops + 1);
...
void _LoopResolver::_ComputeExtrapValueOffset()
{
    ...
    // Calculate loop iteration counts from repeat frames.
    const TsTime period = lp.GetPeriod();
    const int numPreLoops = (period > 0) ?
        static_cast<int>(lp.GetPreRepeatFrames() / period) : 0;
    const int numPostLoops = (period > 0) ?
    ...
    else
    {
        ...
        lastValue =
            ...
            + (numPostLoops + 1) * lp.GetValueOffset();
    }
...
{
    const TsLoopParams &lp = _data->loopParams;
    const TsTime protoSpan = lp.GetMasterInterval().GetSize();
    ...
    if (_aspect == Ts_EvalValue)
    {
        const double offset = shiftIters * lp.GetValueOffset();
    ...
    std::string* reason)
{
    ...
    GfInterval masterInterval, loopedInterval;
    if (haveInnerLoops) {
        masterInterval = data->loopParams.GetMasterInterval();
        loopedInterval = data->loopParams.GetLoopedInterval();
        if (loopedInterval.Contains(atTime) &&
            !masterInterval.Contains(atTime))
        {
