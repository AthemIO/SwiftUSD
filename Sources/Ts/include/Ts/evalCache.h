...
template<typename T> class Ts_Bezier {
 public:
  Ts_Bezier() {}
...
template<typename T> Ts_Bezier<T>::Ts_Bezier(const TsTime time[4], const T value[4])
{
  timePoints[0] = time[0];
  timePoints[1] = time[1];
  timePoints[2] = time[2];
...
template<typename T> void Ts_Bezier<T>::DerivePolynomial()
{
  timeCoeff[0] = timePoints[0];
  timeCoeff[1] = -3.0 * timePoints[0] + 3.0 * timePoints[1];
  timeCoeff[2] = 3.0 * timePoints[0] + -6.0 * timePoints[1] + 3.0 * timePoints[2];
  timeCoeff[3] = -1.0 * timePoints[0] + 3.0 * timePoints[1] + -3.0 * timePoints[2] + timePoints[3];
  valueCoeff[0] = valuePoints[0];
  valueCoeff[1] = -3.0 * valuePoints[0] + 3.0 * valuePoints[1];
  valueCoeff[2] = 3.0 * valuePoints[0] + -6.0 * valuePoints[1] + 3.0 * valuePoints[2];
  valueCoeff[3] = -1.0 * valuePoints[0] + 3.0 * valuePoints[1] + -3.0 * valuePoints[2] +
                  valuePoints[3];
}
...
class Ts_UntypedEvalCache {
 ...
 protected:
  ...
  template<typename T>
  static void _SetupBezierGeometry(TsTime *timePoints,
                                   T *valuePoints,
...
template<typename T, bool INTERPOLATABLE = TsTraits<T>::interpolatable> class Ts_EvalCache;

template<typename T> class Ts_EvalQuaternionCache : public Ts_UntypedEvalCache {
 protected:
  static_assert(std::is_same<T, GfQuatf>::value || std::is_same<T, GfQuatd>::value,
...
template<> class Ts_EvalCache<GfQuatf, true> final : public Ts_EvalQuaternionCache<GfQuatf> {
 public:
  Ts_EvalCache(const Ts_EvalCache<GfQuatf, true> *rhs) : Ts_EvalQuaternionCache<GfQuatf>(rhs) {}
...
template<> class Ts_EvalCache<GfQuatd, true> final : public Ts_EvalQuaternionCache<GfQuatd> {
 public:
  Ts_EvalCache(const Ts_EvalCache<GfQuatd, true> *rhs) : Ts_EvalQuaternionCache<GfQuatd>(rhs) {}
...
template<typename T> class Ts_EvalCache<T, false> final : public Ts_UntypedEvalCache {
 public:
...
template<typename T> class Ts_EvalCache<T, true> final : public Ts_UntypedEvalCache {
 public:
                                                 ...
                                                 const Ts_TypedData<T> *kf2)
{
  ...
  // point as held too.
  TsKnotType knotType = (kf1->_knotType == TsKnotHeld) ? TsKnotHeld : kf2->_knotType;
                                             ...
                                             const Ts_TypedData<T> *kf2)
{
  switch (kf1->_knotType) {
    ...
    case TsKnotLinear:
      return (1.0 / 3.0) * (2.0 * kf1->_GetRightValue() +
                            (kf2->_isDual ? kf2->_GetLeftValue() : kf2->_GetRightValue()));

    case TsKnotBezier:
      return kf1->_GetRightValue() + kf1->_rightTangentLength * kf1->_GetRightTangentSlope();
  }
}
                                             ...
                                             const Ts_TypedData<T> *kf2)
{
  ...
  switch (kf2->_knotType) {
    ...
    case TsKnotLinear:
      return (1.0 / 3.0) * (kf1->_GetRightValue() +
                            2.0 * (kf2->_isDual ? kf2->_GetLeftValue() : kf2->_GetRightValue()));
...
template<typename T> Ts_EvalCache<T, false>::Ts_EvalCache(const Ts_EvalCache<T, false> *rhs)
{
  _value = rhs->_value;
}

template<typename T>
Ts_EvalCache<T, false>::Ts_EvalCache(const Ts_TypedData<T> *kf1, const Ts_TypedData<T> *kf2)
{
  if (!kf1 || !kf2) {
...
template<typename T>
Ts_EvalCache<T, false>::Ts_EvalCache(const TsKeyFrame &kf1, const TsKeyFrame &kf2)
{
  ...
  Ts_TypedData<T> *data = static_cast<Ts_TypedData<T> const *>(Ts_GetKeyFrameData(kf1));

  _value = data->_GetRightValue();
}

template<typename T> VtValue Ts_EvalCache<T, false>::Eval(TsTime t) const
{
  return VtValue(TypedEval(t));
}

template<typename T> VtValue Ts_EvalCache<T, false>::EvalDerivative(TsTime t) const
{
  return VtValue(TypedEvalDerivative(t));
}

template<typename T> T Ts_EvalCache<T, false>::TypedEval(TsTime) const
{
  return _value;
}

template<typename T> T Ts_EvalCache<T, false>::TypedEvalDerivative(TsTime) const
{
...
template<typename T> Ts_EvalCache<T, true>::Ts_EvalCache(const Ts_EvalCache<T, true> *rhs)
{
...
template<typename T>
Ts_EvalCache<T, true>::Ts_EvalCache(const Ts_TypedData<T> *kf1, const Ts_TypedData<T> *kf2)
{
  _Init(kf1, kf2);
}

template<typename T>
Ts_EvalCache<T, true>::Ts_EvalCache(const TsKeyFrame &kf1, const TsKeyFrame &kf2)
{
  // Cast to the correct typed data.  This is a private class, and we assume
  // callers are passing only keyframes from the same spline, and correctly
  // arranging our T to match.
...
template<typename T>
void Ts_EvalCache<T, true>::_Init(const Ts_TypedData<T> *kf1, const Ts_TypedData<T> *kf2)
{
  if (!kf1 || !kf2) {
    TF_CODING_ERROR("Constructing an Ts_EvalCache from invalid keyframes");
    return;
...
template<typename T> VtValue Ts_EvalCache<T, true>::Eval(TsTime t) const
{
  return VtValue(TypedEval(t));
}

template<typename T> VtValue Ts_EvalCache<T, true>::EvalDerivative(TsTime t) const
{
  return VtValue(TypedEvalDerivative(t));
}

template<typename T> T Ts_EvalCache<T, true>::TypedEval(TsTime time) const
{
  if (!_interpolate)
    return _value;
...
template<typename T> T Ts_EvalCache<T, true>::TypedEvalDerivative(TsTime time) const
{
...
template<typename T> const Ts_Bezier<T> *Ts_EvalCache<T, true>::GetBezier() const
{
  return &_cache;
}
                                                                  ...
                                                                  const TsKeyFrame &kf2)
{
  ...
  // callers are passing only keyframes from the same spline, and correctly
  // arranging our T to match.
  return static_cast<const Ts_TypedData<T> *>(Ts_GetKeyFrameData(kf1))
      ->CreateTypedEvalCache(Ts_GetKeyFrameData(kf2));
}

template<typename T>
std::shared_ptr<Ts_EvalCache<T, false>> Ts_EvalCache<T, false>::New(const TsKeyFrame &kf1,
                                                                    const TsKeyFrame &kf2)
{
  // Cast to the correct typed data.  This is a private class, and we assume
  // callers are passing only keyframes from the same spline, and correctly
...
template<typename T>
Ts_EvalQuaternionCache<T>::Ts_EvalQuaternionCache(const Ts_EvalQuaternionCache<T> *rhs)
{
...
template<typename T>
Ts_EvalQuaternionCache<T>::Ts_EvalQuaternionCache(const Ts_TypedData<T> *kf1,
                                                  const Ts_TypedData<T> *kf2)
{
  _Init(kf1, kf2);
}
...
template<typename T>
Ts_EvalQuaternionCache<T>::Ts_EvalQuaternionCache(const TsKeyFrame &kf1, const TsKeyFrame &kf2)
{
  // Cast to the correct typed data.  This is a private class, and we assume
  // callers are passing only keyframes from the same spline, and correctly
  // arranging our T to match.
...
template<typename T>
void Ts_EvalQuaternionCache<T>::_Init(const Ts_TypedData<T> *kf1, const Ts_TypedData<T> *kf2)
{
  if (!kf1 || !kf2) {
    TF_CODING_ERROR(
...
template<typename T> VtValue Ts_EvalQuaternionCache<T>::Eval(TsTime t) const
{
  return VtValue(TypedEval(t));
}

template<typename T> T Ts_EvalQuaternionCache<T>::TypedEval(TsTime time) const
{
...
template<typename T> VtValue Ts_EvalQuaternionCache<T>::EvalDerivative(TsTime t) const
{
  return VtValue(TypedEvalDerivative(t));
}

template<typename T> T Ts_EvalQuaternionCache<T>::TypedEvalDerivative(TsTime) const
{
  return TsTraits<T>::zero;
}
