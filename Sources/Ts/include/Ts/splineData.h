    ...
    const double scale)
{
    ...
    const GfInterval &masterInterval = loopParams.GetMasterInterval();
    if (masterInterval.GetMax() > masterInterval.GetMin())
    {
        // Process start and end times (absolute).
        // In v25.11, we need to reconstruct the TsLoopParams with scaled values
        const TsTime newStart = masterInterval.GetMin() * scale + offset;
        const TsTime newEnd = masterInterval.GetMax() * scale + offset;
        const TsTime newPeriod = newEnd - newStart;

        loopParams = TsLoopParams(
            loopParams.GetLooping(),
            newStart,
            newPeriod,
            loopParams.GetPreRepeatFrames() * scale,  // Relative time, scale only
            loopParams.GetRepeatFrames() * scale,     // Relative time, scale only
            loopParams.GetValueOffset()
        );
    }
