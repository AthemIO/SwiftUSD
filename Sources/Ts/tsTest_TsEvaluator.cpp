    ...
    const TfType& valueType) const
{
    ...
    if (loop.enabled)
    {
        const double period = loop.protoEnd - loop.protoStart;
        TsLoopParams lp(
            true,                           // looping
            loop.protoStart,                // start
            period,                         // period
            loop.numPreLoops * period,      // preRepeatFrames
            loop.numPostLoops * period,     // repeatFrames
            loop.valueOffset                // valueOffset
        );
    ...
    const TsSpline &splineIn) const
{
    ...
    if (splineIn.HasInnerLoops())
    {
        ...
        dataLp.enabled = true;
        dataLp.protoStart = lp.GetStart();
        dataLp.protoEnd = lp.GetStart() + lp.GetPeriod();
        // Calculate numPreLoops from preRepeatFrames / period
        // Note: May not be exact if period doesn't divide evenly
        dataLp.numPreLoops = static_cast<int>(
            std::round(lp.GetPreRepeatFrames() / lp.GetPeriod()));
        dataLp.numPostLoops = static_cast<int>(
            std::round(lp.GetRepeatFrames() / lp.GetPeriod()));
        dataLp.valueOffset = lp.GetValueOffset();
