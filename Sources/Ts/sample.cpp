...
_Sampler::_Init()
{
    ...
    if (_haveInnerLoops)
    {
        const GfInterval masterInterval = _data->loopParams.GetMasterInterval();
        _firstInnerProto = masterInterval.GetMin();
        _lastInnerProto = masterInterval.GetMax();
...
_Sampler::_UnrollInnerLoops()
{
    ...
    const TsTime protoSpan = lp.GetPeriod();
    ...
    for (int loopIndex = -preLoops; loopIndex <= postLoops; ++loopIndex) {
        ...
        double valueOffset = lp.GetValueOffset() * loopIndex;
