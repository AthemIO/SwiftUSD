    ...
    size_t* const firstProtoIndexOut) const
{
    ...
    if (loopParams.GetMasterInterval().GetMax() <= loopParams.GetMasterInterval().GetMin())
    {
        return false;
    }

    // Must have nonzero loop count in at least one direction.
    if (!loopParams.GetPreRepeatFrames() && !loopParams.GetRepeatFrames())
    {
        return false;
    }
    ...
    const auto it =
        std::lower_bound(times.begin(), times.end(), loopParams.GetMasterInterval().GetMin());
    if (it == times.end() || *it != loopParams.GetMasterInterval().GetMin())
    {
        return false;
    }
