    ...
    const TsLoopParams &params)
{
    ...
    // Store a copy, clamping negative repeat frames to zero.
    const double preRepeatFrames = std::max(0.0, params.GetPreRepeatFrames());
    const double repeatFrames = std::max(0.0, params.GetRepeatFrames());
...
std::ostream& operator<<(std::ostream& out, const TsSpline &spline)
{
    ...
    if (spline.HasInnerLoops())
    {
        ...
        out << "Loop:" << std::endl
            << "  start " << TfStringify(masterInterval.GetMin())
            << ", end " << TfStringify(masterInterval.GetMax())
            << ", preRepeatFrames " << lp.GetPreRepeatFrames()
            << ", repeatFrames " << lp.GetRepeatFrames()
            << ", valueOffset " << TfStringify(lp.GetValueOffset())
            << std::endl;
    }
