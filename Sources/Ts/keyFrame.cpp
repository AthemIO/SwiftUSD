                       ...
                       TsTime rightTangentLength)
{
  ...
  _Initialize(
      time, knotType, leftTangentSlope, rightTangentSlope, leftTangentLength, rightTangentLength);
}
...
void TsKeyFrame::_Initialize(const TsTime &time,
                             TsKnotType knotType,
                             const VtValue &leftTangentSlope,
                             const VtValue &rightTangentSlope,
                             TsTime leftTangentLength,
...
void TsKeyFrame::_InitializeKnotType(TsKnotType knotType)
{
  if (!IsInterpolatable() && knotType != TsKnotHeld) {
    knotType = TsKnotHeld;
  }
  else if (IsInterpolatable() && !SupportsTangents() && knotType == TsKnotBezier) {
...
void TsKeyFrame::_InitializeTangentLength(TsTime left, TsTime right)
{
  if (SupportsTangents()) {
...
TsKeyFrame &TsKeyFrame::operator=(const TsKeyFrame &rhs)
{
  if (this != &rhs) {
    _holder.Destroy();
...
bool TsKeyFrame::operator==(const TsKeyFrame &rhs) const
{
...
bool TsKeyFrame::IsEquivalentAtSide(const TsKeyFrame &keyFrame, TsSide side) const
{
  if (GetKnotType() != keyFrame.GetKnotType() || GetTime() != keyFrame.GetTime() ||
...
TsKnotType TsKeyFrame::GetKnotType() const
{
  return _holder.Get()->GetKnotType();
}

void TsKeyFrame::SetKnotType(TsKnotType newType)
{
  _holder.GetMutable()->SetKnotType(newType);
}

bool TsKeyFrame::CanSetKnotType(TsKnotType newType, std::string *reason) const
{
  return _holder.Get()->CanSetKnotType(newType, reason);
}

VtValue TsKeyFrame::GetValue() const
{
  return _holder.Get()->GetValue();
}

VtValue TsKeyFrame::GetLeftValue() const
{
  return _holder.Get()->GetLeftValue();
}

void TsKeyFrame::SetValue(VtValue val)
{
...
VtValue TsKeyFrame::GetValueDerivative() const
{
  return _holder.Get()->GetValueDerivative();
}

VtValue TsKeyFrame::GetZero() const
{
  return _holder.Get()->GetZero();
}

void TsKeyFrame::SetLeftValue(VtValue val)
{
  _holder.GetMutable()->SetLeftValue(val);
}

VtValue TsKeyFrame::GetLeftValueDerivative() const
{
  return _holder.Get()->GetLeftValueDerivative();
}

bool TsKeyFrame::GetIsDualValued() const
{
  return _holder.Get()->GetIsDualValued();
}

void TsKeyFrame::SetIsDualValued(bool isDual)
{
  _holder.GetMutable()->SetIsDualValued(isDual);
}

bool TsKeyFrame::IsInterpolatable() const
{
  return _holder.Get()->ValueCanBeInterpolated();
}

bool TsKeyFrame::IsExtrapolatable() const
{
  return _holder.Get()->ValueCanBeExtrapolated();
}

bool TsKeyFrame::SupportsTangents() const
{
  return _holder.Get()->ValueTypeSupportsTangents();
}

bool TsKeyFrame::HasTangents() const
{
  return _holder.Get()->HasTangents();
}

TsTime TsKeyFrame::GetLeftTangentLength() const
{
  return _holder.Get()->GetLeftTangentLength();
}

VtValue TsKeyFrame::GetLeftTangentSlope() const
{
  return _holder.Get()->GetLeftTangentSlope();
}

TsTime TsKeyFrame::GetRightTangentLength() const
{
  return _holder.Get()->GetRightTangentLength();
}

VtValue TsKeyFrame::GetRightTangentSlope() const
{
  return _holder.Get()->GetRightTangentSlope();
}

bool TsKeyFrame::_ValidateTangentSetting() const
{
  if (!SupportsTangents()) {
    TF_CODING_ERROR("value type %s does not support tangents", GetValue().GetTypeName().c_str());
    return false;
  }
...
void TsKeyFrame::SetLeftTangentLength(TsTime newLen)
{
...
void TsKeyFrame::SetLeftTangentSlope(VtValue newSlope)
{
  if (!_ValidateTangentSetting())
    return;
...
void TsKeyFrame::SetRightTangentLength(TsTime newLen)
{
  if (!_ValidateTangentSetting())
    return;
...
void TsKeyFrame::SetRightTangentSlope(VtValue newSlope)
{
  if (!_ValidateTangentSetting())
    return;
...
bool TsKeyFrame::GetTangentSymmetryBroken() const
{
  return _holder.Get()->GetTangentSymmetryBroken();
}

void TsKeyFrame::SetTangentSymmetryBroken(bool broken)
{
  if (!_ValidateTangentSetting())
    return;
...
void TsKeyFrame::ResetTangentSymmetryBroken()
{
  if (!_ValidateTangentSetting())
    return;
...
static std::string _GetValue(const TsKeyFrame &val)
{
  if (val.GetIsDualValued()) {
    return TfStringify(val.GetLeftValue()) + " - " + TfStringify(val.GetValue());
  }
...
std::ostream &operator<<(std::ostream &out, const TsKeyFrame &val)
{
  if (val.SupportsTangents()) {
    return out << "Ts.KeyFrame(" << val.GetTime() << ", " << _GetValue(val) << ", "
               << val.GetKnotType() << ", " << val.GetLeftTangentSlope() << ", "
               << val.GetRightTangentSlope() << ", " << val.GetLeftTangentLength() << ", "
