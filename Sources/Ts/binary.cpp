    ...
    const std::unordered_map<TsTime, VtDictionary>** const customDataOut)
{
    ...
    if (hasLoops)
    {
        ...
        const double start = lp.GetStart();
        const double period = lp.GetPeriod();
        const int32_t numPreLoops = static_cast<int32_t>(
            std::round(lp.GetPreRepeatFrames() / period));
        const int32_t numPostLoops = static_cast<int32_t>(
            std::round(lp.GetRepeatFrames() / period));

        _WriteBytes<double>(buf, start);                    // protoStart
        _WriteBytes<double>(buf, start + period);           // protoEnd
        _WriteBytes<int32_t>(buf, numPreLoops);
        _WriteBytes<int32_t>(buf, numPostLoops);
        _WriteBytes<double>(buf, lp.GetValueOffset());
    }
    ...
    std::unordered_map<TsTime, VtDictionary> &&customData)
{
    ...
    if (hasLoops)
    {
        double protoStart, protoEnd, valueOffset;
        int32_t numPreLoops, numPostLoops;
        ...
        const double period = protoEnd - protoStart;
        data->loopParams = TsLoopParams(
            true,                      // looping
            protoStart,                // start
            period,                    // period
            numPreLoops * period,      // preRepeatFrames
            numPostLoops * period,     // repeatFrames
