...
TsSpline_KeyFrames ::TsSpline_KeyFrames(TsSpline_KeyFrames const &other,
    ...
    : _extrapolation(other._extrapolation), _loopParams(other._loopParams)
{
  if (keyFrames) {
...
void TsSpline_KeyFrames::SetKeyFrames(const TsKeyFrameMap &keyFrames)
{
...
void TsSpline_KeyFrames::SetKeyFrame(TsKeyFrame kf, GfInterval *intervalAffected)
{
  ...
  if (_loopParams.GetLooping()) {
    ...
    GfInterval loopedInterval = _loopParams.GetLoopedInterval();
    GfInterval masterInterval = _loopParams.GetMasterInterval();
    ...
    if (inMaster) {
      ...
      _UnrollKeyFrameRange(&_loopedKeyFrames, k0, k, _loopParams, &times);
    }
...
void TsSpline_KeyFrames::RemoveKeyFrame(TsTime t, GfInterval *intervalAffected)
{
  ...
  if (_loopParams.GetLooping()) {
    ...
    if (inMaster) {
      ...
      for (int i = -numPrepeats; i <= numRepeats; i++) {
        ...
        if (intervalAffected) {
          *intervalAffected |= _FindRemoveKeyFrameChangedInterval(time);
        }
...
void TsSpline_KeyFrames::_LoopParamsChanged(bool loopingChanged,
...
void TsSpline_KeyFrames::_SetNormalFromLooped()
{
  ...
  // the looped keys
  _normalKeyFrames.erase(_normalKeyFrames.begin(),
                         _normalKeyFrames.lower_bound(loopedInterval.GetMin()));
  _normalKeyFrames.insert(_loopedKeyFrames.begin(),
                          _loopedKeyFrames.lower_bound(loopedInterval.GetMin()));

  // Clear the masterInterval region and copy the corresponding from the
  // looped keys
  _normalKeyFrames.erase(_normalKeyFrames.lower_bound(masterInterval.GetMin()),
                         _normalKeyFrames.lower_bound(masterInterval.GetMax()));
  _normalKeyFrames.insert(_loopedKeyFrames.lower_bound(masterInterval.GetMin()),
                          _loopedKeyFrames.lower_bound(masterInterval.GetMax()));

  // Clear the region after the repeat and copy the corresponding from
  // the looped keys
  _normalKeyFrames.erase(_normalKeyFrames.lower_bound(loopedInterval.GetMax()),
                         _normalKeyFrames.end());
  _normalKeyFrames.insert(_loopedKeyFrames.lower_bound(loopedInterval.GetMax()),
                          _loopedKeyFrames.end());
}

void TsSpline_KeyFrames::_SetLoopedFromNormal()
{
...
bool TsSpline_KeyFrames::operator==(const TsSpline_KeyFrames &rhs) const
{
  ...
  if (_extrapolation != rhs._extrapolation || _loopParams != rhs._loopParams) {
    return false;
  }
...
void TsSpline_KeyFrames::_UnrollMaster()
{
...
void TsSpline_KeyFrames::_UnrollKeyFrames(TsKeyFrameMap *keyFrames, const TsLoopParams &params)
{
  // Get Loop domain intervals
  ...
  keyFrames->erase(keyFrames->lower_bound(loopedInterval.GetMin()),
                   keyFrames->lower_bound(masterInterval.GetMin()));

  // Clear the keys in the repeat range
  keyFrames->erase(keyFrames->lower_bound(masterInterval.GetMax()),
                   keyFrames->lower_bound(loopedInterval.GetMax()));

  // Iterators for the masterInterval keys to propagate
  TsKeyFrameMap::iterator k0 = keyFrames->lower_bound(masterInterval.GetMin());
  TsKeyFrameMap::iterator k1 = keyFrames->lower_bound(masterInterval.GetMax());

  _UnrollKeyFrameRange(keyFrames, k0, k1, params);
}

void TsSpline_KeyFrames::_UnrollKeyFrameRange(TsKeyFrameMap *keyFrames,
    ...
    for (TsKeyFrameMap::iterator k = k0; k != k1; k++) {
      ...
      if (v.IsHolding<double>()) {
        ...
        if (key.GetIsDualValued()) {
          key.SetLeftValue(VtValue(key.GetLeftValue().Get<double>() + valueOffset));
        }
      }
...
TsKeyFrameMap *TsSpline_KeyFrames::_GetKeyFramesMutable()
{
  return const_cast<TsKeyFrameMap *>(&GetKeyFrames());
}

TsSpline_KeyFrames::_KeyFrameRange TsSpline_KeyFrames::_GetKeyFrameRange(TsTime time)
{
...
TsSpline_KeyFrames::_KeyFrameRange TsSpline_KeyFrames::_GetKeyFrameRange(TsTime leftTime,
                                                                         TsTime rightTime)
{
  // Get the keyframe before leftTime
  TsKeyFrameMap::iterator i = _GetKeyFramesMutable()->lower_bound(leftTime);
  if (i != _GetKeyFramesMutable()->begin()) {
    --i;
  }

  // Get the keyframe after rightTime
  TsKeyFrameMap::iterator j = _GetKeyFramesMutable()->upper_bound(rightTime);

  return std::make_pair(i, j);
}

GfInterval TsSpline_KeyFrames::_GetTimeInterval(TsTime t)
{
  ...
  std::pair<TsKeyFrameMap::const_iterator, TsKeyFrameMap::const_iterator> range(
      GetKeyFrames().lower_bound(t), GetKeyFrames().upper_bound(t));
...
GfInterval TsSpline_KeyFrames::_FindRemoveKeyFrameChangedInterval(TsTime time)
{
  ...
  if (r.GetMin() == -std::numeric_limits<TsTime>::infinity()) {
    ...
    TsKeyFrameMap::const_iterator it = GetKeyFrames().find(r.GetMin());
    if (it != GetKeyFrames().end() && it->GetKnotType() == TsKnotHeld) {
...
GfInterval TsSpline_KeyFrames::_FindSetKeyFrameChangedInterval(const TsKeyFrame &keyFrame)
{
  ...
  if (r.GetMin() == -std::numeric_limits<TsTime>::infinity()) {
    ...
    if (aExtrapLeft == bExtrapLeft) {
      ...
      if (firstKeyFrame.GetLeftValue() == keyFrame.GetLeftValue()) {
        ...
        // keyframes
                 ...
                 firstKeyFrame.GetLeftTangentSlope() == keyFrame.GetLeftTangentSlope())
        {
          r.SetMin(time, /* closed */ false);
        }
      }
    ...
    TsKeyFrameMap::const_iterator it = keyFrames.find(r.GetMin());
    ...
    if (aExtrapRight == bExtrapRight) {
      ...
      if (lastKeyFrame.GetValue() == keyFrame.GetValue()) {
        ...
        // slopes match, do we not have a change after the last keyframes
                 ...
                 lastKeyFrame.GetRightTangentSlope() == keyFrame.GetRightTangentSlope())
        {
          r.SetMax(time, /* closed */ false);
        }
      }
...
TsExtrapolationType TsSpline_KeyFrames::_GetEffectiveExtrapolationType(const TsKeyFrame &keyFrame,
                                                                       const TsSide &side) const
{
  return Ts_GetEffectiveExtrapolationType(
      keyFrame, GetExtrapolation(), GetKeyFrames().size() == 1, side);
}
...
void TsSpline_KeyFrames::SetLoopParams(const TsLoopParams &params)
{
  ...
  bool valueOffsetChanged = params.GetValueOffset() != _loopParams.GetValueOffset();
