...
void TsTypeRegistry::InitializeDataHolder(Ts_PolymorphicDataHolder *holder, const VtValue &value)
{
  ...
  if (ARCH_UNLIKELY(i == _dataFactoryMap.end())) {
    PlugPluginPtr plugin = PlugRegistry::GetInstance().GetPluginForType(value.GetType());
    if (plugin) {
      plugin->Load();
    ...
    if (i == _dataFactoryMap.end()) {
      TF_CODING_ERROR("cannot create keyframes of type %s", value.GetTypeName().c_str());
...
bool TsTypeRegistry::IsSupportedType(const TfType &type) const
{
