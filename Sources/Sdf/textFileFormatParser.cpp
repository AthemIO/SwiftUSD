...
// #include "Pegtl/contrib/trace.hpp" // Commented out - may not be needed
#include "Ar/asset.h"
...
template<> constexpr auto errorMessage<LeftBracket> = "Expected [";
template<> constexpr auto errorMessage<RightBracket> = "Expected ]";
...
template<> constexpr auto errorMessage<Digit> = "Expected number [0-9]";
...
constexpr auto errorMessage<ListOf<TupleValueItem>> =
    ...
    "separated by ,";
template<>
constexpr auto errorMessage<TupleValueItem> =
...
constexpr auto errorMessage<ListOf<ListValueItem>> =
    ...
    "separated by ,";
template<>
constexpr auto errorMessage<ListValueItem> =
    "Expected number, identifier, string, asset ref, list or tuple";
template<> constexpr auto errorMessage<TokenSeparator> = "Expected spaces";
template<> constexpr auto errorMessage<DictionaryKey> = "Expected string or identifier";
template<> constexpr auto errorMessage<DictionaryValue> = "Expected dictionary";
template<> constexpr auto errorMessage<DictionaryValueClose> = "Expected }";
template<> constexpr auto errorMessage<ListOf<String>> = "Expected list of strings separated by ,";
template<> constexpr auto errorMessage<String> = "Expected string";
template<> constexpr auto errorMessage<StringDictionaryOpen> = "Expected {";
template<> constexpr auto errorMessage<StringDictionaryClose> = "Expected }";
template<> constexpr auto errorMessage<StringDictionaryItem> = "Expected string : string";
template<> constexpr auto errorMessage<Identifier> = "Expected identifier";
template<> constexpr auto errorMessage<MetadataClose> = "Expected )";
...
template<>
constexpr auto errorMessage<ConnectValue> =
    "Expected None, path ref, or list of path refs separated by ,";
template<> constexpr auto errorMessage<RelationshipAssignmentClose> = "Expected ]";
template<> constexpr auto errorMessage<RelationshipTargetClose> = "Expected ]";
template<> constexpr auto errorMessage<RelocatesMapOpen> = "Expected {";
template<> constexpr auto errorMessage<RelocatesMapClose> = "Expected }";
template<>
constexpr auto errorMessage<ReferenceList> =
    "Expected None, reference, or list of references separated by ,";
template<>
constexpr auto errorMessage<PayloadList> =
    "Expected None, payload, or list of payloads separated by ,";
template<>
constexpr auto errorMessage<InheritsOrSpecializesList> =
    "Expected None, path ref, or list of path refs separated by ,";
template<>
constexpr auto errorMessage<NameList> =
    "Expected string or list of strings enclosed in [] separated by ,";
template<> constexpr auto errorMessage<VariantStatementOpen> = "Expected {";
template<> constexpr auto errorMessage<VariantStatementClose> = "Expected }";
template<> constexpr auto errorMessage<VariantStatementListOpen> = "Expected {";
template<> constexpr auto errorMessage<VariantStatementListClose> = "Expected }";
template<>
...
template<>
constexpr auto errorMessage<PrimContents> =
    "Expected child order, property order, prim, property, or variant set";
template<> constexpr auto errorMessage<SublayerListClose> = "Expected ]";
template<> constexpr auto errorMessage<NoneOrTypedListValue> = "Expected None or [";

////////////////////////////////////////////////////////////////////////
// Common Keyword actions

template<> struct TextParserAction<KeywordNone> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordAnimationBlock> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    Sdf_TextParserCurrentParsingContext parsingContext = context.parsingContext.back();
...
static int64_t _TextToInteger(const Input &in, Sdf_TextParserContext &context)
{
  ...
  const bool looksValid = (!str.empty() && isDigit(str[0])) ||
  ...
  if (outOfRange) {
    errMsg = TfStringPrintf("Integer '%s' out-of-range for int64_t", in.string().c_str());
  }

  if (!errMsg.empty()) {
    Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errMsg);
    throw PEGTL_NS::parse_error(errMsg, in);
  }
  return val;
}

template<int N> struct TextParserAction<ArrayEditReferenceIndex<N>> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<ArrayEditSizeArg> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    // Evaluate the integer value and store it in \p context.
...
template<> struct TextParserAction<KeywordFill> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<ArrayEditPrepend> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    ...
    if (writeRef) {
      context.arrayEditFactory->PrependRef(context.arrayEditReferenceIndexes[0]);
    }
    else {
...
template<> struct TextParserAction<ArrayEditAppend> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    // If we have an index for arg 0 this is writing a reference, otherwise
    // writing a literal.
    const bool writeRef = context.arrayEditReferencePresence & 1;
    if (writeRef) {
      context.arrayEditFactory->AppendRef(context.arrayEditReferenceIndexes[0]);
    }
    else {
...
template<> struct TextParserAction<ArrayEditWrite> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    // If we have an index for arg 0 this is writing a reference, otherwise
    // writing a literal.
    const bool writeRef = context.arrayEditReferencePresence & 1;
    if (writeRef) {
      context.arrayEditFactory->WriteRef(context.arrayEditReferenceIndexes[0],
                                         context.arrayEditReferenceIndexes[1]);
    }
    else {
      context.arrayEditFactory->Write(context.currentValue, context.arrayEditReferenceIndexes[1]);
    }
  }
};

template<> struct TextParserAction<ArrayEditInsert> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    ...
    if (insertRef) {
      context.arrayEditFactory->InsertRef(context.arrayEditReferenceIndexes[0],
                                          context.arrayEditReferenceIndexes[1]);
    }
    else {
      context.arrayEditFactory->Insert(context.currentValue, context.arrayEditReferenceIndexes[1]);
    }
  }
};

template<> struct TextParserAction<ArrayEditErase> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    TF_DEV_AXIOM(context.arrayEditReferencePresence == 1);
    context.arrayEditFactory->EraseRef(context.arrayEditReferenceIndexes[0]);
  }
};

template<> struct TextParserAction<ArrayEditMinSize> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.arrayEditHasFill) {
      context.arrayEditFactory->MinSizeFill(context.arrayEditSizeArg, context.currentValue);
    }
    else {
...
template<> struct TextParserAction<ArrayEditResize> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.arrayEditHasFill) {
      context.arrayEditFactory->SetSizeFill(context.arrayEditSizeArg, context.currentValue);
    }
    else {
...
template<> struct TextParserAction<ArrayEditMaxSize> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<ArrayEditInstruction> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordEdit> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    ...
    const SdfValueTypeName arrayType = SdfSchema::GetInstance().FindType(
    ...
    const std::string &scalarTypeName = arrayType.GetScalarType().GetAsToken();

    auto editFactory = Sdf_ParserHelpers::MakeArrayEditFactoryForMenvaName(scalarTypeName);
    if (!editFactory) {
      error(TfStringPrintf("Unsupported array edit for type '%s'", scalarTypeName.c_str()));
    }
...
template<> struct TextParserAction<ArrayEditValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    VtValue arrayEdit = context.arrayEditFactory->FinalizeAndReset();
    if (arrayEdit.IsEmpty()) {
      std::string errMsg = TfStringPrintf("Failed to produce VtArrayEdit instance: %s",
    ...
    const SdfValueTypeName scalarType = SdfSchema::GetInstance().FindType(
        context.values.valueTypeName);
    context.values.SetupFactory(scalarType.GetArrayType().GetAsToken());
  }
};

template<> struct TextParserAction<KeywordCustomData> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordSymmetryArguments> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    // if the current context is Metadata, this signals we need
    // to start a key value metadata production
...
template<> struct TextParserAction<KeywordPermission> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordSymmetryFunction> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    // if we are in metadata, this opens a new parsing context
...
template<> struct TextParserAction<KeywordDisplayUnit> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    // if we are in metadata, this opens a new parsing context
    // for symmetry function metadata
...
template<> struct TextParserAction<KeywordCustom> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<String> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<typename T> struct _Bundler {
  void operator()(const double valueIn, VtValue *const valueOut)
  {
    *valueOut = VtValue(static_cast<T>(valueIn));
  }
};

static VtValue _BundleSplineValue(Sdf_TextParserContext &context,
                                  const Sdf_ParserHelpers::Value &value)
{
  VtValue result;
  TsDispatchToValueTypeTemplate<_Bundler>(
      context.spline.GetValueType(), value.Get<double>(), &result);
  return result;
}

static void _SetSplineTangent(Sdf_TextParserContext &context,
                              const double width,
                              const VtValue &slope,
...
template<class Input>
std::pair<bool, Sdf_ParserHelpers::Value> _HelperGetNumericValueFromString(
    const Input &in, Sdf_TextParserContext &context)
{
  const std::pair<bool, Sdf_ParserHelpers::Value> result = _GetNumericValueFromString(
      in.string_view());
  if (result.first) {
    TF_WARN(
        "Integer literal '%s' on line %zu%s%s out of range, "
...
template<> struct TextParserAction<Number> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    const std::pair<bool, Sdf_ParserHelpers::Value> result = _HelperGetNumericValueFromString(
        in, context);
...
template<> struct TextParserAction<Identifier> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    Sdf_TextParserCurrentParsingContext parsingContext = context.parsingContext.back();
...
template<> struct TextParserAction<ArrayType> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    Sdf_TextParserCurrentParsingContext parsingContext = context.parsingContext.back();
    if (parsingContext == Sdf_TextParserCurrentParsingContext::DictionaryTypeName) {
...
template<> struct TextParserAction<NamespacedName> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::AttributeSpec) {
      std::string errorMessage;
...
template<> struct TextParserAction<PathRef> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::ConnectAttribute) {
      ...
      if (absPath.ContainsPrimVariantSelection()) {
        TF_WARN(
            "Connection path <%s> (in file @%s@, line %zu) has a "
             ...
             Sdf_TextParserCurrentParsingContext::RelationshipDefault)
    {
      ...
      if (!context.relParsingNewTargetChildren.empty()) {
        ...
        context.data->Set(
            context.path, SdfChildrenKeys->RelationshipTargetChildren, VtValue(children));
      }
             ...
             Sdf_TextParserCurrentParsingContext::RelocatesMetadata)
    {
      ...
      if (!context.seenFirstRelocatesPath) {
        ...
        if (!SdfSchema::IsValidRelocatesSourcePath(context.relocatesKey)) {
          const std::string errorMessage = "'" + context.relocatesKey.GetAsString() +
...
template<> struct TextParserAction<AssetRef> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() ==
            Sdf_TextParserCurrentParsingContext::ReferencesListOpMetadata ||
            ...
            Sdf_TextParserCurrentParsingContext::PayloadListOpMetadata)
    {
      ...
      std::string evaluatedAssetPath = Sdf_EvalAssetPath(
          inputStrView.data(), inputStrView.length(), isTripleDelimited);
...
template<> struct TextParserAction<NumberValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    const std::pair<bool, Sdf_ParserHelpers::Value> result = _HelperGetNumericValueFromString(
        in, context);
...
template<> struct TextParserAction<IdentifierValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<StringValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    const std::string_view inputStrView = in.string_view();
    ...
    std::string evaluatedString = Sdf_EvalQuotedString(
        inputStrView.data(), inputStrView.length(), numDelimeters);
...
template<> struct TextParserAction<AssetRefValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<AtomicValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<PathRefValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<TypedValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<NameList> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
             ...
             Sdf_TextParserCurrentParsingContext::VariantSetsMetadata)
    {
      ...
      if (!_SetListOpItemsWithError(
              SdfFieldKeys->VariantSetNames, context.listOpType, names, context, errorMessage))
      {
        Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);
      ...
      if (context.listOpType == SdfListOpTypeAdded || context.listOpType == SdfListOpTypeExplicit)
      {
        for (const auto &i : context.nameVector) {
          context.data->CreateSpec(context.path.AppendVariantSelection(i, ""),
                                   SdfSpecTypeVariantSet);
        }

        context.data->Set(
            context.path, SdfChildrenKeys->VariantSetChildren, VtValue(context.nameVector));
      }
...
template<> struct TextParserAction<TupleValueOpen> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<TupleValueClose> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<TypedTupleValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    std::string errorMessage;
...
template<> struct TextParserAction<ListValueOpen> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<ListValueClose> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<TypedListValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    std::string errorMessage;
...
template<> struct TextParserAction<EmptyListValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    // it's an array type
...
template<> struct TextParserAction<DictionaryValueOpen> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<DictionaryValueClose> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordDictionary> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<DictionaryType> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    ...
    if (!context.values.SetupFactory(context.dictionaryTypeName)) {
      std::string errorMessage = "Unrecognized value typename '" + context.dictionaryTypeName +
                                 "' for dictionary";
      Sdf_TextFileFormatParser_Err(context, in.input(), in.position(), errorMessage);
...
template<> struct TextParserAction<DictionaryKey> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    ...
    if (TfStringStartsWith(dictionaryKey, "\"") || TfStringStartsWith(dictionaryKey, "'")) {
      ...
      dictionaryKey = Sdf_EvalQuotedString(
          dictionaryKey.c_str(), dictionaryKey.length(), numDelimeters);
    }
...
template<> struct TextParserAction<DictionaryValueItem> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<DictionaryValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::KeyValueMetadata) {
...
template<> struct TextParserAction<StringDictionaryOpen> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<StringDictionaryClose> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    context.currentDictionaries.pop_back();
  }
...
template<> struct TextParserAction<MetadataOpen> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<MetadataClose> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    _PopContext(context);
  }
...
template<> struct TextParserAction<KeyValueMetadata> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    Sdf_TextParserCurrentParsingContext specContext = context.parsingContext.rbegin()[1];
    SdfSpecType specType = _GetSpecTypeFromContext(specContext);
...
template<> struct TextParserAction<GeneralListOpMetadata> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    Sdf_TextParserCurrentParsingContext specContext = context.parsingContext.rbegin()[1];
    SdfSpecType specType = _GetSpecTypeFromContext(specContext);
...
template<> struct TextParserAction<ListOpKeyValueMetadata> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    Sdf_TextParserCurrentParsingContext specContext = context.parsingContext.rbegin()[1];
    SdfSpecType specType = _GetSpecTypeFromContext(specContext);
...
template<> struct TextParserAction<KeywordDoc> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<DocMetadata> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<SymmetryFunctionMetadata> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    ...
    context.data->Set(context.path,
                      ...
                      context.symmetryFunctionName.empty() ?
                          VtValue(TfToken()) :
...
template<> struct TextParserAction<KeywordAdd> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::Metadata) {
...
template<> struct TextParserAction<KeywordDelete> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::Metadata) {
...
template<> struct TextParserAction<KeywordAppend> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::Metadata) {
...
template<> struct TextParserAction<KeywordPrepend> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::Metadata) {
...
template<> struct TextParserAction<KeywordReorder> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::Metadata) {
...
template<> struct TextParserAction<Sdf_PathParser::Dot> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordVarying> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordConfig> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordUniform> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    context.variability = VtValue(SdfVariabilityUniform);
  }
...
template<> struct TextParserAction<AttributeVariability> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<AttributeType> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<AttributeAssignmentOptional> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (!context.values.valueTypeIsValid) {
      context.values.StopRecordingString();
...
template<> struct TextParserAction<AttributeSpec> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordTimeSamples> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<TimeSampleMap> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordSpline> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<SplineItem> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordBezier> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordHermite> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    // We landed on a curve item, pop the anticipated SplineItem context
    _PopContext(context);
...
template<> struct TextParserAction<KeywordNone_LC> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() ==
    ...
    else if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineTangent) {
      context.splineTangentAlgorithm = TsTangentAlgorithmNone;
    }
  }
};

template<> struct TextParserAction<KeywordAutoEase> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineTangent) {
...
template<> struct TextParserAction<KeywordHeld> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() ==
            Sdf_TextParserCurrentParsingContext::SplinePostExtrapItem ||
...
template<> struct TextParserAction<KeywordLinear> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() ==
            Sdf_TextParserCurrentParsingContext::SplinePostExtrapItem ||
...
template<> struct TextParserAction<KeywordCurve> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordSloped> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<SlopeValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    const std::pair<bool, Sdf_ParserHelpers::Value> result = _HelperGetNumericValueFromString(
        in, context);
...
template<> struct TextParserAction<SplineLoopItemProtoStart> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    const std::pair<bool, Sdf_ParserHelpers::Value> result = _HelperGetNumericValueFromString(
        in, context);
...
template<> struct TextParserAction<SplineLoopItemProtoEnd> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    const std::pair<bool, Sdf_ParserHelpers::Value> result = _HelperGetNumericValueFromString(
        in, context);
...
template<> struct TextParserAction<SplineLoopItemNumPreLoops> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    const std::pair<bool, Sdf_ParserHelpers::Value> result = _HelperGetNumericValueFromString(
        in, context);
...
template<> struct TextParserAction<SplineLoopItemNumPostLoops> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    const std::pair<bool, Sdf_ParserHelpers::Value> result = _HelperGetNumericValueFromString(
        in, context);
...
template<> struct TextParserAction<SplineLoopItemValueOffset> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    const std::pair<bool, Sdf_ParserHelpers::Value> result = _HelperGetNumericValueFromString(
        in, context);
...
template<> struct TextParserAction<KeywordLoop> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<SplineLoopItem> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    ...
    // Use TsLoopParams constructor: TsLoopParams(bool looping, TsTime start, TsTime period,
    // TsTime preRepeatFrames, TsTime repeatFrames, double valueOffset)
    TsLoopParams lp(
        true,                                                   // looping
        context.splineLoopItem[0],                              // start (protoStart)
        context.splineLoopItem[1] - context.splineLoopItem[0],  // period (protoEnd - protoStart)
        context.splineLoopItem[2],                              // preRepeatFrames (numPreLoops)
        context.splineLoopItem[3],                              // repeatFrames (numPostLoops)
        context.splineLoopItem[4]                               // valueOffset
    );
...
template<> struct TextParserAction<KeywordRepeat> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordReset> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineKeywordLoop) {
...
template<> struct TextParserAction<KeywordOscillate> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineKeywordLoop) {
...
template<> struct TextParserAction<SplineKnotValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    const std::pair<bool, Sdf_ParserHelpers::Value> result = _HelperGetNumericValueFromString(
        in, context);
...
template<> struct TextParserAction<SplineKnotPreValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    const std::pair<bool, Sdf_ParserHelpers::Value> result = _HelperGetNumericValueFromString(
        in, context);
...
template<> struct TextParserAction<SplineKnotTime> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    const std::pair<bool, Sdf_ParserHelpers::Value> result = _HelperGetNumericValueFromString(
        in, context);
    context.splineKnot = TsKnot(context.spline.GetValueType());
...
template<> struct TextParserAction<SplineKnotItem> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<SplineKnotValueWithoutPreValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<SplineKnotValueWithPreValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<SplineKnotValues> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<SplineKnotParam> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<SplineTangentWidthSlopeAlgorithmItem> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    // Set the parsed tangent value
    _SetSplineTangent(context,
...
template<> struct TextParserAction<SplineTangentWidthSlopeItem> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    // Set the parsed tangent value
    _SetSplineTangent(context,
...
template<> struct TextParserAction<SplineTangentSlopeAlgorithmItem> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    // Set the parsed tangent value
    _SetSplineTangent(context,
...
template<> struct TextParserAction<SplineTangentSlopeItem> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<SplineTangentSlope> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    const std::pair<bool, Sdf_ParserHelpers::Value> result = _HelperGetNumericValueFromString(
        in, context);
...
template<> struct TextParserAction<SplineTangentWidth> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    const std::pair<bool, Sdf_ParserHelpers::Value> result = _HelperGetNumericValueFromString(
        in, context);
...
template<> struct TextParserAction<SplineTangent> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    _PopContext(context);
  }
...
template<> struct TextParserAction<KeywordPre> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineKnotItem) {
      // We had anticipated getting a SplineKnotItem, but we did not get
...
template<> struct TextParserAction<SplinePreExtrapItem> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordPost> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::SplineKnotItem) {
      // We had anticipated getting a SplineKnotItem, but we did not get
...
template<> struct TextParserAction<SplinePostExtrapItem> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<SplineInterpMode> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<SplinePostShaping> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<SplineValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.splineValid) {
...
template<> struct TextParserAction<KeywordConnect> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<ConnectValue> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordRel> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() == Sdf_TextParserCurrentParsingContext::AttributeSpec) {
...
template<> struct TextParserAction<RelationshipSpec> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordDefault> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<Assignment> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<RelationshipAssignmentOptional> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<RelationshipAssignmentOpen> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    context.relParsingTargetPaths = SdfPathVector();
  }
...
template<> struct TextParserAction<RelationshipTargetOpen> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<RelationshipTargetClose> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<PropertySpec> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<PrimMetadataOptional> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<PrimSpec> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    // store the names of our children
    if (!context.nameChildrenStack.back().empty()) {
      context.data->Set(
          context.path, SdfChildrenKeys->PrimChildren, VtValue(context.nameChildrenStack.back()));
    }
...
template<> struct TextParserAction<KeywordDef> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordClass> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordOver> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordKind> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KindMetadata> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordRelocates> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    context.relocatesKey = SdfPath();
    context.seenFirstRelocatesPath = false;
...
template<> struct TextParserAction<RelocatesMapClose> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    ...
    if (specContext == Sdf_TextParserCurrentParsingContext::PrimSpec) {
      ...
      context.data->Set(
          context.path, SdfFieldKeys->LayerRelocates, VtValue(context.relocatesParsing));
    }
    else {
      Sdf_TextFileFormatParser_Err(
          context, in.input(), in.position(), "Unable to parse relocates data, unknown context!");
    }
...
template<> struct TextParserAction<KeywordPayload> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<PayloadList> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<PayloadListItem> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordReferences> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    // we may have seen a list op keyword, which
    // would have put us in the listop parsing
...
template<> struct TextParserAction<ReferenceParametersOpen> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<ReferenceParametersClose> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<ReferenceListItem> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordOffset> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordScale> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<ReferenceList> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordSpecializes> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordInherits> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    // we may have seen a list op keyword, which would
    // have put us in the listop parsing context
...
template<> struct TextParserAction<InheritsOrSpecializesList> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() ==
        Sdf_TextParserCurrentParsingContext::InheritsListOpMetadata)
    {
      ...
      std::string errorMessage;
      if (!_SetListOpItemsWithError(SdfFieldKeys->InheritPaths,
                                    context.listOpType,
             ...
             Sdf_TextParserCurrentParsingContext::SpecializesListOpMetadata)
    {
      ...
      std::string errorMessage;
      if (!_SetListOpItemsWithError(SdfFieldKeys->Specializes,
                                    context.listOpType,
...
template<> struct TextParserAction<KeywordVariants> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<VariantsMetadata> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordVariantSets> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordVariantSet> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<VariantStatementOpen> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<VariantStatementClose> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<VariantStatementListOpen> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<VariantStatementListClose> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<VariantStatement> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<VariantSetStatement> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    ...
    if (!context.data->HasSpec(variantSetPath)) {
      ...
      std::vector<TfToken> vec = context.data->GetAs<std::vector<TfToken>>(
...
template<> struct TextParserAction<KeywordNameChildren> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordProperties> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<ChildOrPropertyOrderStatement> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordPrefixSubstitutions> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordSuffixSubstitutions> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<PrefixOrSuffixSubstitutionsMetadata> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (context.parsingContext.back() ==
        Sdf_TextParserCurrentParsingContext::PrefixSubstitutionsMetadata)
...
template<> struct TextParserAction<LayerHeader> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    ...
    if (TfStringStartsWith(header, expected)) {
      ...
      std::string reason;
      SdfFileVersion layerVersion = SdfUsdaData::ValidateLayerVersionString(versionStr, &reason);
      if (layerVersion) {
      ...
      const std::string reason = TfStringPrintf(
          "Invalid layer header '%s' does not start with '%s'", header.c_str(), expected.c_str());
      throw PEGTL_NS::parse_error(reason, in);
    }
...
template<> struct TextParserAction<LayerSpec> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<KeywordRootPrims> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<LayerKeyValueMetadata> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    Sdf_TextParserCurrentParsingContext specContext = context.parsingContext.rbegin()[1];
    SdfSpecType specType = _GetSpecTypeFromContext(specContext);
...
template<> struct TextParserAction<KeywordSubLayers> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<SublayerItem> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
...
template<> struct TextParserAction<SublayerListClose> {
  template<class Input> static void apply(const Input &in, Sdf_TextParserContext &context)
  {
    if (!context.subLayerPaths.empty()) {
      context.data->Set(
          SdfPath::AbsoluteRootPath(), SdfFieldKeys->SubLayers, VtValue(context.subLayerPaths));
                              ...
                              VtValue *outputValue)
{
  ...
  context.values.errorReporter =
      [&context, capture0 = std::cref(content), errPtr = &error](auto &&PH1) mutable {
        *errPtr = true;
        return Sdf_TextFileFormatParser ::_ReportParseError<PegtlInput>(
  ...
  try {
    ...
    status = Sdf_TextFileFormatParser::PEGTL_NS::parse<
        Sdf_TextFileFormatParser::PEGTL_NS::internal::must<
    ...
    if (status && context.currentValue.IsEmpty()) {
      TF_CODING_ERROR(
          "Parse succeeded but output value was not "
          "populated");
      return false;
    }
                    ...
                    SdfLayerHints *hints)
{
  ...
  if (!contentPtr) {
    TF_RUNTIME_ERROR("Failed to read asset content: @%s@", fileContext.c_str());
    return false;
  }
  ...
  try {
    if (!metadataOnly) {
      // Parse with tracing disabled (standard_trace not available in this PEGTL version)
      status = Sdf_TextFileFormatParser::PEGTL_NS::parse<
          Sdf_TextFileFormatParser::PEGTL_NS::internal::must<
    ...
    Sdf_TextFileFormatParser::Sdf_TextFileFormatParser_Err(
        context, content, content.position(), "Internal layer parser error.");
